...
class Enemy {
  ...
  update(dt) {
    ...
    const collisionXRules =
      (parseInt(this.x + 66) > player.x && parseInt(this.x) < player.x) ||
      (parseInt(this.x + 66) > player.x + 60 &&
        parseInt(this.x) < player.x + 66)
...
class Player {
  ...
  handleInput(e) {
    ...
    const moveRight =
      e === 'right' && this.x !== 405 && this.controllable === true
    const moveDown =
      e === 'down' && this.y !== 403 && this.controllable === true
...
const playerDeathSound = new Howl({
  src: [
    'https://s3.amazonaws.com/nuclearlaunchdetected/mp3/Hellion_Death03.mp3'
  ]
})

let victorySound = new Howl({
  src: [svcYesLinks[returnRandomLink()]]
})
